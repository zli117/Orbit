# RUOK - Maintenance Playbook

Operations guide for deploying, maintaining, and backing up the RUOK.
...
Create `/etc/systemd/system/ruok.service`:
...
sudo systemctl enable ruok
sudo systemctl start ruok
```
...
# Create tunnel
cloudflared tunnel create ruok
...
**Note:** Path-based routing requires apps to support running under a subpath. RUOK needs `ORIGIN=https://yourdomain.com/okr` in this case.
...
Create `/opt/ruok/backup.sh`:

```bash
#!/bin/bash
set -e

DB_PATH="/opt/ruok/data/okr.db"
BACKUP_DIR="/opt/ruok/backups"
RETENTION_DAYS=30
...
0 3 * * * /opt/ruok/backup.sh >> /var/log/okr-backup.log 2>&1
```
...
# Sync backups
rclone sync /opt/ruok/backups nextcloud:OKR-Backups/
```
...
sudo useradd -r -m -d /opt/ruok okr
sudo -u okr mkdir -p /opt/ruok

# Clone or copy your app
cd /opt/ruok
sudo -u okr git clone https://github.com/yourusername/ruok.git .
...
sudo -u okr ln -s /mnt/ssd/okr-data /opt/ruok/data

# Reduce memory usage - add to .env
echo "NODE_OPTIONS=--max-old-space-size=512" >> /opt/ruok/.env
...
| Task | Command |
|------|---------|
| Start service | `docker compose up -d` or `sudo systemctl start ruok` |
| Stop service | `docker compose down` or `sudo systemctl stop ruok` |
| View logs | `docker compose logs -f` or `journalctl -u ruok -f` |
| Backup now | `/opt/ruok/backup.sh` |
| Restore backup | `gunzip -c backup.db.gz > data/okr.db` |
| Check DB health | `sqlite3 data/okr.db "PRAGMA integrity_check;"` |
| Update app | `git pull && npm ci && npm run build && sudo systemctl restart ruok` |
